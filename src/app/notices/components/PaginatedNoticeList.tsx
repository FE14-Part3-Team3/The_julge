...
import { GetShopNoticesQuery, SeoulDistrict, seoulDistricts, SortOption } from "@/types/common";
import { Section } from "../../../components/Section/Section";
import { useNoticeService, useSetNoticeQuery } from "@/hooks/api/useParsedNoticeQuery";
...
export default function PaginatedNoticeList() {
  ...
  const [inputQuery, setInputQuery] = useState<GetShopNoticesQuery>(defaultQuery);
  const [selectedDistricts, setSelectedDistricts] = useState<string[]>([]);
  const setQueryToURL = useSetNoticeQuery();
  const { data: allNotices, query, isLoading: isLoadingNotices } = useNoticeService(inputQuery, searchParams);

  useEffect(() => {
    setInputQuery((prev) => ({ ...prev, address: selectedDistricts as SeoulDistrict[] }));
  }, [selectedDistricts]);
  ...
  const applyFilters = () => {
    ...
    setInputQuery({ ...nextQuery, address: selectedDistricts as SeoulDistrict[] });
  ...
  const selectedLabel = OPTIONS.find((opt) => opt.value === (inputQuery.sort ?? "time"))?.label ?? "";
  ...
  return (
    <Section name="전체 공고" className="relative">
      ...
      {/* 공고 끝 */}
      {allNotices?.count === 0 && <EmptyCard description="해당 공고가 없습니다." />}
      <div className="mt-10">
        {/* <Pagination
