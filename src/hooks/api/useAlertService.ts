...
export function useUserAlerts(userId: string | undefined, query: GetListQuery) {
  // 유저의 알림 목록 조회
  return useQuery<GetUserAlertsResponse>({
    queryKey: ["user-alerts", userId, query],
    queryFn: async () => {
      const token = localStorage.getItem("token");
      const res = await requestor.get<GetUserAlertsResponse>(
        `/users/${userId}/alerts`,
        {
          params: query,
          headers: {
...
export const useReadAlert = () => {
  ...
  const mutation = useMutation<
    PutAlertItemResponse,
    Error,
    { userId: string | undefined; alertId: string | undefined }
  >({
    // 알림 읽음 처리
    mutationFn: async ({ userId, alertId }) => {
      const token = localStorage.getItem("token");
      const res = await requestor.put<PutAlertItemResponse>(
